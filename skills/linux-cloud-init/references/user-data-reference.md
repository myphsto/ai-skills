...
users:
  ...
  - name: admin                   # create a new admin
    gecos: Server Admin       # real name in /etc/passwd
    ...
    ssh_authorized_keys:
      - ssh-ed25519 AAAAC3Nz... admin@laptop
      - ssh-rsa AAAAB3Nz... admin@backup
  - name: deploy
...
final_message: "Server bootstrap complete."
```
...
power_state:
  mode: reboot
  delay: now
  message: "Rebooting so admin's docker group membership applies"
```
