...
| **`.env` files + credential files** | Irreplaceable secrets | separate encrypted archive, rotate with a documented procedure |
...
- **`/var/log/`** — logs must **ship off the host live** using whatever
  shipping tooling exists (rsync, rclone, syslog), not be caught by a backup
  after the fact. If ransomware deletes your logs, the backup is too late.
...
the **`linux-archive-integrity`** skill. Hard-linked
incremental rsync snapshots live in
**`linux-rsync-sync`**. This file keeps only the
