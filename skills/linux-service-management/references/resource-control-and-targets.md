...
`/etc/systemd/system.control/<unit>.d/50-*.conf` so the limit survives
reboots; with `--runtime` it goes to `/run/systemd/system.control/<unit>.d/`
instead. Inspect with `systemctl cat <unit>` or
`systemctl show <unit> -p DropInPaths`.
...
systemctl show reindexer.service -p CPUWeight -p CPUQuotaPerSecUSec -p MemoryMax -p Nice
