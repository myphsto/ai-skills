...
Gotcha: `OnUnitActiveSec=` has no reference point until the paired service
has actually run once — until then `systemctl list-timers` shows `n/a` for
the next elapse and the timer never fires. If the service may not have run
yet, add `OnBootSec=` or `OnActiveSec=` as a first-run anchor.
...
**`%` gotcha:** an unescaped `%` in the cron command is converted to a
newline, and everything after the first `%` is fed to the job as stdin
(`crontab(5)`). So `* * * * * date +%s > /tmp/t.log` runs only `date +`
and the rest becomes stdin — the job appears to "run" while doing
something else. Escape as `\%` when you need a literal percent
(e.g. `date +\%s`).
