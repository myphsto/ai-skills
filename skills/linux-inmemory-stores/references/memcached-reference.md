...
```bash
# Create a SASL user (the secret should come from a secrets manager):
sudo saslpasswd2 -a memcached -c appuser
# then add -S to OPTIONS / memcached.conf and restart.
```

Use an **external secrets manager** to manage the SASL password rather than typing it inline.
