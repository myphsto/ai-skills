...
- The task is storing or injecting the auth password itself; supply the
  `requirepass`/SASL secret from a secrets manager or environment, then return
  here to apply it.
...
5. Set authentication (Redis ACL/password or Memcached SASL) before binding to
   any non-loopback address. Pull the secret from a secrets manager or
   environment.
...
redis-cli -a 'CHANGE-ME' CONFIG REWRITE
```
...
reachable off-host. Store the password in a secrets manager or environment,
never inline in a committed config. Deep dives:
...
- Password handling: external secrets manager. Port firewalling: `linux-firewall-ssl`.
