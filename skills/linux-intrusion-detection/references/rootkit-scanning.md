...
- **Baseline discipline mirrors AIDE.** `rkhunter --propupd` is to rkhunter
  ...
  ```
- **Ship the logs off-box.** An attacker with root can edit
  `/var/log/rkhunter.log`. Forward it to a central collector using whatever
  shipping tooling the host already has.
