...
ICMP/redirect hardening) see `sysctl-reference.md` in this skill's
`references/`; keep the two sets in separate drop-in files.
...
Keep performance keys here and SECURITY keys in a separate security drop-in —
never mix the two in one file, so each set has clear ownership.
