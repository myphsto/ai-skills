...
description: "Use when measured Linux performance evidence justifies persistent network, queue, writeback, swap, or congestion-control sysctl changes on Debian/Ubuntu or RHEL-family hosts. Profile first; keep security sysctls out of scope."
license: MIT
...
- The goal is SECURITY hardening (rp_filter, tcp_syncookies, kptr_restrict,
  redirects). Those keys are out of scope for this skill — see
  `references/sysctl-reference.md` for the security set, do not duplicate it
  into performance drop-ins.
...
`kptr_restrict`, `accept_redirects`, core-dump hygiene) are documented in
`references/sysctl-reference.md` in this skill. Keep security keys in a
separate security drop-in and performance keys in this skill's drop-in; do
not move or duplicate either set.
